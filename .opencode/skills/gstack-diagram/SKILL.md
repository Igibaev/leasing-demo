...
```bash
BUNDLE=""
for c in "$HOME/.opencode/skills/gstack/lib/diagram-render/dist/diagram-render.html" \
         "$(git rev-parse --show-toplevel 2>/dev/null)/lib/diagram-render/dist/diagram-render.html"; do
  [ -f "$c" ] && BUNDLE="$c" && break
done
[ -z "$BUNDLE" ] && echo "BUNDLE_MISSING — run: cd ~/.config/opencode/skills/gstack && bun run build:diagram-render" && exit 1
...
bun run ~/.config/opencode/skills/gstack/bin/gstack-render.ts "<staged>" --wait-selector '#done' \
