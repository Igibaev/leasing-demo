...
- **Auth failure:** If stderr contains "auth", "login", "unauthorized", or "API key": "Codex authentication failed. Run \`codex login\` to authenticate." Fall back to opencode (in-host) subagent.
- **Timeout:** "Codex timed out after 5 minutes." Fall back to opencode (in-host) subagent.
- **Empty response:** "Codex returned no response." Fall back to opencode (in-host) subagent.

On any Codex error, fall back to the opencode (in-host) subagent below.
...
If opencode (in-host) subagent ran:
```
SECOND OPINION (opencode (in-host) subagent):
════════════════════════════════════════════════════════════
...
5. **Cross-model synthesis:** After presenting the second opinion output, provide 3-5 bullet synthesis:
   - Where opencode (in-host) agrees with the second opinion
   - Where opencode (in-host) disagrees and why
   - Whether the challenged premise changes opencode (in-host)'s recommendation
...
_ROOT=$(git rev-parse --show-toplevel 2>/dev/null)
D=""
[ -n "$_ROOT" ] && [ -x "$_ROOT/.opencode/skills/gstack/design/dist/design" ] && D="$_ROOT/.opencode/skills/gstack/design/dist/design"
...
> "Want outside design perspectives on the chosen approach? Codex proposes a visual thesis, content plan, and interaction ideas. A opencode (in-host) subagent proposes an alternative aesthetic direction."
>
...
For this phase (design-sketch), retain the historical review-log skill identifier. Add `"host":"opencode","outside_provider":"codex","outside_status":"completed|unavailable|disabled|skipped","phase":"design-sketch"`. Record each attempted pass separately when outcomes differ. Use `source:"codex"` only for completed external CLI output, and `source:"in-host"` for a native pass. Historical `source:"claude"` continues to mean a native Claude subagent. CLI availability or a native fallback does not count as outside completion. Preserve reported modelUsage, including multiple models; unknown model identity stays unknown.

2. **opencode (in-host) subagent** (via Agent tool, `run_in_background: false` — subagents default to background since Claude Code v2.1.198):
"For this product approach, what design direction would you recommend? What aesthetic, typography, and interaction patterns fit? What would make this approach feel inevitable to the user? Be specific — font names, hex colors, spacing values."

Present Codex output under `CODEX SAYS (design sketch):` and subagent output under `OPENCODE (IN-HOST) SUBAGENT (design direction):`.
...
```bash
~/.config/opencode/skills/gstack/bin/gstack-developer-profile --log-session '{"date":"TIMESTAMP","mode":"MODE","project_slug":"SLUG","signal_count":N,"signals":SIGNALS_ARRAY,"design_doc":"DOC_PATH","assignment":"ASSIGNMENT_TEXT","resources_shown":[],"topics":TOPICS_ARRAY}' 2>/dev/null || true
```
...
1. **Scan at sink first.** The repo copy leaves the private store, so scan the
   ...
   `~/.config/opencode/skills/gstack/bin/gstack-redact --from-file <tmp>`; exit 3
...
```bash
PROFILE=$(~/.config/opencode/skills/gstack/bin/gstack-builder-profile 2>/dev/null) || PROFILE="SESSION_COUNT: 0
TIER: introduction"
...
```bash
~/.config/opencode/skills/gstack/bin/gstack-config get founder_resources 2>/dev/null || echo "true"
```
...
`~/.config/opencode/skills/gstack/bin/gstack-config set founder_resources false`, then
...
```bash
eval "$(~/.config/opencode/skills/gstack/bin/gstack-slug 2>/dev/null || true)"
~/.config/opencode/skills/gstack/bin/gstack-developer-profile --log-session '{"date":"'"$(date -u +%Y-%m-%dT%H:%M:%SZ)"'","mode":"resources","project_slug":"'"${SLUG:-unknown}"'","signal_count":0,"signals":[],"design_doc":"","assignment":"","resources_shown":["URL1","URL2","URL3"],"topics":[]}' 2>/dev/null || true
```
...
```bash
~/.config/opencode/skills/gstack/bin/gstack-telemetry-log --event-type handoff --skill office-hours --outcome accepted --session-id "$_SESSION_ID" 2>/dev/null || true
```
On D, log declined and stop:
```bash
~/.config/opencode/skills/gstack/bin/gstack-telemetry-log --event-type handoff --skill office-hours --outcome declined --session-id "$_SESSION_ID" 2>/dev/null || true
```
