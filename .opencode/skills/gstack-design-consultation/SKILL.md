...
- `legacy` with `DESIGN_MD_MARKER: none` → ask once (AskUserQuestion): **A) Convert** (recommended; `bun --no-env-file run $GSTACK_BIN/gstack-design-md.ts convert --write` keeps a `.legacy.bak` and every section) **B) Keep legacy** (`bun --no-env-file run $GSTACK_BIN/gstack-design-md.ts mark legacy-keep`; read as prose from now on) **C) Start fresh**. The answer lives in the file, so no skill asks again; a marker already present is obeyed silently.
...
A `PRODUCT.md` (impeccable's product-context file) already answers the product questions below: treat it as the user's prior answers, confirm them in one line, and do not re-ask. Never open `.opencode/skills/impeccable/**` or any other skill's files; PRODUCT.md and DESIGN.md are the shared surface.
...
Use AskUserQuestion:
> "Want outside design voices? Codex proposes an independent design direction; opencode (in-host) subagent does an independent design direction proposal."
>
...
- On any Codex error: proceed with opencode (in-host) subagent output only; identify it as the only completed independent proposal.
- If opencode (in-host) subagent also fails: "Outside voices unavailable — continuing to Phase 3 with my draft direction."

Output headers: `CODEX SAYS (design direction):` and `OPENCODE (IN-HOST) SUBAGENT (design direction):`.
...
Use real token values, no placeholders; omit invented `components` entries. Outside plan mode, after writing DESIGN.md, require `bun --no-env-file run ~/.config/opencode/skills/gstack/bin/gstack-design-md.ts check DESIGN.md` to print `DESIGN_MD_FORMAT: spec`.
