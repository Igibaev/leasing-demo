...
```bash
~/.config/opencode/skills/gstack/bin/gstack-gbrain-detect
```
...
**If A (Retry)**: re-run `~/.config/opencode/skills/gstack/bin/gstack-gbrain-detect`
...
```bash
[ -f ~/.config/opencode/skills/gstack/bin/gstack-code-intelligence ] \
  && bun ~/.config/opencode/skills/gstack/bin/gstack-code-intelligence select gbrain 2>/dev/null \
  || true
```
...
```bash
~/.config/opencode/skills/gstack/bin/gstack-gbrain-install
```
...
```bash
. ~/.config/opencode/skills/gstack/bin/gstack-gbrain-lib.sh
...
```bash
printf '%s' "$GBRAIN_POOLER_URL" | ~/.config/opencode/skills/gstack/bin/gstack-gbrain-supabase-verify -
```
...
```bash
orgs=$(~/.config/opencode/skills/gstack/bin/gstack-gbrain-supabase-provision list-orgs --json)
```
...
```bash
result=$(~/.config/opencode/skills/gstack/bin/gstack-gbrain-supabase-provision \
  create gbrain "$REGION" "$ORG_SLUG" --json)
INFLIGHT_REF=$(echo "$result" | jq -r .ref)
~/.config/opencode/skills/gstack/bin/gstack-gbrain-supabase-provision wait "$INFLIGHT_REF" --json
pooler=$(~/.config/opencode/skills/gstack/bin/gstack-gbrain-supabase-provision \
...
verify_json=$(GBRAIN_MCP_TOKEN="$GBRAIN_MCP_TOKEN" \
  ~/.config/opencode/skills/gstack/bin/gstack-gbrain-mcp-verify "$MCP_URL")
...
```bash
~/.config/opencode/skills/gstack/bin/gstack-gbrain-install || exit $?
...
```bash
current_tier=$(~/.config/opencode/skills/gstack/bin/gstack-gbrain-repo-policy get)
```
...
- `unset` → AskUserQuestion: "How should `<normalized-remote>` interact with
  ...
  ```bash
  ~/.config/opencode/skills/gstack/bin/gstack-gbrain-repo-policy set "$REMOTE" "$TIER"
  ```
...
~/.config/opencode/skills/gstack/bin/gstack-artifacts-init --url-form-supported "$URL_FORM"
~/.config/opencode/skills/gstack/bin/gstack-config set artifacts_sync_mode artifacts-only
...
try:
    ...
    pass
")
~/.config/opencode/skills/gstack/bin/gstack-gbrain-source-wireup --strict \
...
```bash
bun run ~/.config/opencode/skills/gstack/bin/gstack-memory-ingest.ts --probe
```
...
bulk via `bun run ~/.config/opencode/skills/gstack/bin/gstack-memory-ingest.ts --bulk --quiet`. Set
...
After answer:
```bash
~/.config/opencode/skills/gstack/bin/gstack-config set transcript_ingest_mode <choice>
bun run ~/.config/opencode/skills/gstack/bin/gstack-gbrain-sync.ts --full --no-brain-sync
```
...
hook runs `bun run ~/.config/opencode/skills/gstack/bin/gstack-gbrain-sync.ts --incremental --quiet` on every skill
...
```bash
_HASH=$(~/.config/opencode/skills/gstack/bin/gstack-config endpoint-hash 2>/dev/null)
_POLICY=$(~/.config/opencode/skills/gstack/bin/gstack-config get brain_trust_policy@$_HASH 2>/dev/null || echo unset)
...
```bash
~/.config/opencode/skills/gstack/bin/gstack-config set brain_trust_policy@$_HASH personal
...
```bash
~/.config/opencode/skills/gstack/bin/gstack-config set brain_trust_policy@$_HASH <personal|shared>
```
...
```bash
_CURRENT_SYNC=$(~/.config/opencode/skills/gstack/bin/gstack-config get artifacts_sync_mode 2>/dev/null || echo off)
if [ "$_CURRENT_SYNC" = "off" ]; then
  ~/.config/opencode/skills/gstack/bin/gstack-config set artifacts_sync_mode full
...
```bash
~/.config/opencode/skills/gstack/bin/gstack-gbrain-detect 2>/dev/null || true
~/.config/opencode/skills/gstack/bin/gstack-config get transcript_ingest_mode 2>/dev/null || echo "off"
~/.config/opencode/skills/gstack/bin/gstack-config get artifacts_sync_mode 2>/dev/null || echo "off"
