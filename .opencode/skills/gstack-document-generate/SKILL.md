...
```bash
$GSTACK_BIN/gstack-skill-end --skill "document-generate" --outcome OUTCOME \
...
git diff --cached --no-color | grep '^+' | sed 's/^+//' | \
  ~/.config/opencode/skills/gstack/bin/gstack-redact --repo-visibility "${REDACT_VIS:-unknown}" --json
