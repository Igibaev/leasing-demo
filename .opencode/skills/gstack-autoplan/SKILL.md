...
SLUG=$(~/.config/opencode/skills/gstack/browse/bin/remote-slug 2>/dev/null || basename "$(git rev-parse --show-toplevel 2>/dev/null || pwd)")
...
This is qualitatively different from taste decisions. When opencode (in-host) and Codex both
...
- Phase 1: the sibling registry file `../gstack-plan-ceo-review/SKILL.md`, relative to the installed `/autoplan` SKILL.md directory (local: `.opencode/skills/gstack-plan-ceo-review/SKILL.md`; global: `~/.config/opencode/skills/gstack-plan-ceo-review/SKILL.md`)
- Phase 2: the sibling registry file `../gstack-plan-design-review/SKILL.md`, relative to the installed `/autoplan` SKILL.md directory (local: `.opencode/skills/gstack-plan-design-review/SKILL.md`; global: `~/.config/opencode/skills/gstack-plan-design-review/SKILL.md`) (only if UI scope detected)
- Phase 2.5: the sibling registry file `../gstack-plan-devex-review/SKILL.md`, relative to the installed `/autoplan` SKILL.md directory (local: `.opencode/skills/gstack-plan-devex-review/SKILL.md`; global: `~/.config/opencode/skills/gstack-plan-devex-review/SKILL.md`) (only if DX scope detected)
- Phase 3: the sibling registry file `../gstack-plan-eng-review/SKILL.md`, relative to the installed `/autoplan` SKILL.md directory (local: `.opencode/skills/gstack-plan-eng-review/SKILL.md`; global: `~/.config/opencode/skills/gstack-plan-eng-review/SKILL.md`)
...
- Dual voices: always run BOTH opencode (in-host) subagent AND Codex if available (P6).
  Run opencode (in-host) first, then Codex, sequentially;
...
```
  ...
  **opencode (in-host) CEO subagent** (via Agent tool):
...
(CEO — strategy challenge) and opencode (in-host) SUBAGENT (CEO — strategic independence).
Produce CEO consensus table:

```
CEO DUAL VOICES — CONSENSUS TABLE:
═══════════════════════════════════════════════════════════════
  Dimension                           opencode (in-host)  Codex  Consensus
  ──────────────────────────────────── ─────── ─────── ─────────
  1. Premises valid?                   —       —      —
...
**Phase 1 complete.**
Codex: [completed: N concerns / unavailable / disabled]. opencode (in-host) subagent: [completed: N issues / unavailable].
...
- [ ] CEO dual voices ran (Codex + opencode (in-host) subagent, or noted unavailable)
...
```
  ...
  **opencode (in-host) design subagent** (native tool):
  Claude Code: set Agent `run_in_background: false` if its schema exposes it.
  Other hosts: foreground; await completion when supported.
...
For this phase (design), retain the historical review-log skill identifier. Add `"host":"opencode","outside_provider":"codex","outside_status":"completed|unavailable|disabled|skipped","phase":"design"`. Record each attempted pass separately when outcomes differ. Use `source:"codex"` only for completed external CLI output, and `source:"in-host"` for a native pass. Historical `source:"claude"` continues to mean a native Claude subagent. CLI availability or a native fallback does not count as outside completion. Preserve reported modelUsage, including multiple models; unknown model identity stays unknown.
...
2. Step 0.5 (Dual Voices): Present the completed calls above under Codex SAYS (design — UX challenge)
   and opencode (in-host) SUBAGENT (design — independent review).
...
```
  ...
  **opencode (in-host) DX subagent** (native tool):
  Claude Code: set Agent `run_in_background: false` if its schema exposes it.
  Other hosts: foreground; await completion when supported.
...
For this phase (dx), retain the historical review-log skill identifier. Add `"host":"opencode","outside_provider":"codex","outside_status":"completed|unavailable|disabled|skipped","phase":"dx"`. Record each attempted pass separately when outcomes differ. Use `source:"codex"` only for completed external CLI output, and `source:"in-host"` for a native pass. Historical `source:"claude"` continues to mean a native Claude subagent. CLI availability or a native fallback does not count as outside completion. Preserve reported modelUsage, including multiple models; unknown model identity stays unknown.
...
2. Step 0.5 (Dual Voices): Present the completed calls above under Codex SAYS
   (DX — developer experience challenge) and opencode (in-host) SUBAGENT (DX — independent review).
...
```
  ...
  **opencode (in-host) eng subagent** (native tool):
  Claude Code: set Agent `run_in_background: false` if its schema exposes it.
  Other hosts: foreground; await completion when supported.
...
For this phase (eng), retain the historical review-log skill identifier. Add `"host":"opencode","outside_provider":"codex","outside_status":"completed|unavailable|disabled|skipped","phase":"eng"`. Record each attempted pass separately when outcomes differ. Use `source:"codex"` only for completed external CLI output, and `source:"in-host"` for a native pass. Historical `source:"claude"` continues to mean a native Claude subagent. CLI availability or a native fallback does not count as outside completion. Preserve reported modelUsage, including multiple models; unknown model identity stays unknown.
...
2. Step 0.5 (Dual Voices): Present the completed calls above under Codex SAYS
   (eng — architecture challenge) and opencode (in-host) SUBAGENT (eng — independent review).
...
- [ ] Dual voices ran (Codex + opencode (in-host) subagent, or noted unavailable)
- [ ] CEO consensus table produced
...
```bash
eval "$(~/.config/opencode/skills/gstack/bin/gstack-slug 2>/dev/null)"
...
### Review Scores
- CEO: [summary]
- CEO Voices: Codex [summary], opencode (in-host) subagent [summary], Consensus [X/6 confirmed]
- Design: [summary or "skipped, no UI scope"]
- Design Voices: Codex [summary], opencode (in-host) subagent [summary], Consensus [X/7 confirmed] (or "skipped")
- Eng: [summary]
- Eng Voices: Codex [summary], opencode (in-host) subagent [summary], Consensus [X/6 confirmed]
- DX: [summary or "skipped, no developer-facing scope"]
- DX Voices: Codex [summary], opencode (in-host) subagent [summary], Consensus [X/6 confirmed] (or "skipped")
...
~/.config/opencode/skills/gstack/bin/gstack-review-log '{"skill":"plan-ceo-review","timestamp":"'"$TIMESTAMP"'","status":"STATUS","unresolved":N,"critical_gaps":N,"mode":"SELECTIVE_EXPANSION","via":"autoplan","commit":"'"$COMMIT"'"}'

~/.config/opencode/skills/gstack/bin/gstack-review-log '{"skill":"plan-eng-review","timestamp":"'"$TIMESTAMP"'","status":"STATUS","unresolved":N,"critical_gaps":N,"issues_found":N,"mode":"FULL_REVIEW","via":"autoplan","commit":"'"$COMMIT"'"}'
```

If Phase 2 ran (UI scope):
```bash
~/.config/opencode/skills/gstack/bin/gstack-review-log '{"skill":"plan-design-review","timestamp":"'"$TIMESTAMP"'","status":"STATUS","unresolved":N,"via":"autoplan","commit":"'"$COMMIT"'"}'
```

If Phase 2.5 ran (DX scope):
```bash
~/.config/opencode/skills/gstack/bin/gstack-review-log '{"skill":"plan-devex-review","timestamp":"'"$TIMESTAMP"'","status":"STATUS","initial_score":N,"overall_score":N,"product_type":"TYPE","tthw_current":"TTHW","tthw_target":"TARGET","unresolved":N,"via":"autoplan","commit":"'"$COMMIT"'"}'
```
...
```bash
~/.config/opencode/skills/gstack/bin/gstack-review-log '{"skill":"autoplan-voices","run_id":"AUTOPLAN_RUN_ID","timestamp":"'"$TIMESTAMP"'","status":"STATUS","source":"SOURCE","host":"opencode","outside_provider":"codex","outside_status":"OUTSIDE_STATUS","phase":"ceo","via":"autoplan","consensus_confirmed":N,"consensus_disagree":N,"commit":"'"$COMMIT"'"}'

~/.config/opencode/skills/gstack/bin/gstack-review-log '{"skill":"autoplan-voices","run_id":"AUTOPLAN_RUN_ID","timestamp":"'"$TIMESTAMP"'","status":"STATUS","source":"SOURCE","host":"opencode","outside_provider":"codex","outside_status":"OUTSIDE_STATUS","phase":"eng","via":"autoplan","consensus_confirmed":N,"consensus_disagree":N,"commit":"'"$COMMIT"'"}'
```
...
```bash
~/.config/opencode/skills/gstack/bin/gstack-review-log '{"skill":"autoplan-voices","run_id":"AUTOPLAN_RUN_ID","timestamp":"'"$TIMESTAMP"'","status":"STATUS","source":"SOURCE","host":"opencode","outside_provider":"codex","outside_status":"OUTSIDE_STATUS","phase":"design","via":"autoplan","consensus_confirmed":N,"consensus_disagree":N,"commit":"'"$COMMIT"'"}'
```
...
```bash
~/.config/opencode/skills/gstack/bin/gstack-review-log '{"skill":"autoplan-voices","run_id":"AUTOPLAN_RUN_ID","timestamp":"'"$TIMESTAMP"'","status":"STATUS","source":"SOURCE","host":"opencode","outside_provider":"codex","outside_status":"OUTSIDE_STATUS","phase":"dx","via":"autoplan","consensus_confirmed":N,"consensus_disagree":N,"commit":"'"$COMMIT"'"}'
```
...
For this phase (autoplan), retain the historical review-log skill identifier. Add `"host":"opencode","outside_provider":"codex","outside_status":"completed|unavailable|disabled|skipped","phase":"autoplan"`. Record each attempted pass separately when outcomes differ. Use `source:"codex"` only for completed external CLI output, and `source:"in-host"` for a native pass. Historical `source:"claude"` continues to mean a native Claude subagent. CLI availability or a native fallback does not count as outside completion. Preserve reported modelUsage, including multiple models; unknown model identity stays unknown.
