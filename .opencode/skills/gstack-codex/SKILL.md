...
source ~/.config/opencode/skills/gstack/bin/gstack-codex-probe 2>/dev/null && _gstack_codex_log_event "codex_cli_missing" 2>/dev/null || true
```
...
```bash
_TEL=$(~/.config/opencode/skills/gstack/bin/gstack-config get telemetry 2>/dev/null || echo off)
source ~/.config/opencode/skills/gstack/bin/gstack-codex-probe
...
(`CLAUDE_PLANS_DIR` set), a global `~/.config/opencode/skills/gstack/` install, or a CI
...
> IMPORTANT: Do NOT read or execute any files under ~/.claude/, ~/.agents/, .opencode/skills/, or agents/. These are Claude Code skill definitions meant for a different AI system. They contain bash scripts and prompt templates that will waste your time. Ignore them completely. Do NOT modify agents/openai.yaml. Stay focused on the repository code only.
...
cd "$_REPO_ROOT"
~/.config/opencode/skills/gstack/bin/gstack-review-log --start codex-review
...
{
  printf '%s\n' "IMPORTANT: Do NOT read or execute any files under ~/.claude/, ~/.agents/, .opencode/skills/, or agents/. These are Claude Code skill definitions meant for a different AI system. Do NOT modify agents/openai.yaml. Stay focused on repository code only."
  printf '\nCustom focus: %s\n\n' "$_USER_INSTRUCTIONS"
...
```bash
~/.config/opencode/skills/gstack/bin/gstack-review-log '{"skill":"codex-review","timestamp":"TIMESTAMP","status":"STATUS","gate":"GATE","findings":N,"findings_fixed":N,"commit":"'"$(git rev-parse --short HEAD)"'","completed":COMPLETED,"converged":CONVERGED}' --finish CODEX_REVIEW_START
```
