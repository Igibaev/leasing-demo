...
```bash
$GSTACK_BIN/gstack-question-log '{"skill":"sync-gbrain","question_id":"<id>","question_summary":"<short>","category":"<approval|clarification|routing|cherry-pick|feedback-loop>","door_type":"<one-way|two-way>","options_count":N,"user_choice":"<key>","recommended":"<key>","session_id":"SESSION_ID"}' 2>/dev/null || true
```
...
```bash
$GSTACK_BIN/gstack-skill-end --skill "sync-gbrain" --outcome OUTCOME \
...
`$GSTACK_BIN/gstack-gbrain-sync.ts`.
...
```bash
~/.config/opencode/skills/gstack/bin/gstack-gbrain-detect 2>/dev/null
```
...
```bash
bun run ~/.config/opencode/skills/gstack/bin/gstack-gbrain-sync.ts <user-args>
```
