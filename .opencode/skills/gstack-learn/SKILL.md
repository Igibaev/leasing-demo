...
```bash
$GSTACK_BIN/gstack-question-log '{"skill":"learn","question_id":"<id>","question_summary":"<short>","category":"<approval|clarification|routing|cherry-pick|feedback-loop>","door_type":"<one-way|two-way>","options_count":N,"user_choice":"<key>","recommended":"<key>","session_id":"SESSION_ID"}' 2>/dev/null || true
```
...
```bash
$GSTACK_BIN/gstack-skill-end --skill "learn" --outcome OUTCOME \
...
~/.config/opencode/skills/gstack/bin/gstack-learnings-search --limit 20 2>/dev/null || echo "No learnings yet."
```
...
```bash
eval "$(~/.config/opencode/skills/gstack/bin/gstack-slug 2>/dev/null)"
~/.config/opencode/skills/gstack/bin/gstack-learnings-search --query "USER_QUERY" --limit 20 2>/dev/null || echo "No matches."
```
...
```bash
eval "$(~/.config/opencode/skills/gstack/bin/gstack-slug 2>/dev/null)"
~/.config/opencode/skills/gstack/bin/gstack-learnings-search --limit 100 2>/dev/null
```
...
```bash
eval "$(~/.config/opencode/skills/gstack/bin/gstack-slug 2>/dev/null)"
~/.config/opencode/skills/gstack/bin/gstack-learnings-search --limit 50 2>/dev/null
```
...
```bash
~/.config/opencode/skills/gstack/bin/gstack-learnings-log '{"skill":"learn","type":"TYPE","key":"KEY","insight":"INSIGHT","confidence":N,"source":"user-stated","files":["FILE1"]}'
```
