...
**Install offer (one question, asked once).** If the probe printed `DESIGN_DETECTOR_INSTALL_OFFER`, Read `$GSTACK_ROOT/design-html/sections/detector-install-offer.md` and follow it before any other step; otherwise skip it.
...
_PRETEXT_VENDOR=""
_ROOT=$(git rev-parse --show-toplevel 2>/dev/null)
[ -n "$_ROOT" ] && [ -f "$_ROOT/.opencode/skills/gstack/design-html/vendor/pretext.js" ] && _PRETEXT_VENDOR="$_ROOT/.opencode/skills/gstack/design-html/vendor/pretext.js"
[ -z "$_PRETEXT_VENDOR" ] && [ -f ~/.config/opencode/skills/gstack/design-html/vendor/pretext.js ] && _PRETEXT_VENDOR=~/.config/opencode/skills/gstack/design-html/vendor/pretext.js
...
```bash
_DJ=$(mktemp); bun --no-env-file run $GSTACK_BIN/gstack-design-detect.ts scan --format gstack --host opencode <finalized.html> > "$_DJ"; echo "DETECT_EXIT_CODE=$?"; echo "DETECT_JSON=$_DJ"
```
...
```bash
bun run ~/.config/opencode/skills/gstack/bin/gstack-render.ts <path-to-finalized.html> \
