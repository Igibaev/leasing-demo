...
```bash
bun --no-env-file run $GSTACK_BIN/gstack-design-detect.ts install --host opencode
```

On **B**, continue without scans. On **C**, run `~/.config/opencode/skills/gstack/bin/gstack-config set design_detector_install_prompted true`. On **D**, run `~/.config/opencode/skills/gstack/bin/gstack-config set design_detector off`. Never pass `--sha256` or `--base` yourself: they exist for maintainers and mirrors. If the user also wants the `/impeccable` skill and its hook, they run `npx impeccable install` themselves; gstack never does.
...
```bash
_TMP=$(mktemp -d); _DUMP=$(cat "$GSTACK_ROOT/lib/dom-dump.js")
...
```bash
_DJ=$(mktemp); bun --no-env-file run $GSTACK_BIN/gstack-design-detect.ts scan --format gstack --host opencode "<REPORT_DIR from Setup>/dom/<RUN_ID>" > "$_DJ"; echo "DETECT_EXIT_CODE=$?"; echo "DETECT_JSON=$_DJ"
```
...
2. **opencode (in-host) design subagent** (Agent tool, `run_in_background: false`; await its result):
...
- On any Codex error: proceed with opencode (in-host) subagent output only, tagged `[single-model]`.
- If opencode (in-host) subagent also fails: "Outside voices unavailable — continuing with primary review."

Output headers: `CODEX SAYS (design source audit):` and `OPENCODE (IN-HOST) SUBAGENT (design consistency):`.
...
**Log the result:**
```bash
$GSTACK_BIN/gstack-review-log '{"skill":"design-outside-voices","timestamp":"'"$(date -u +%Y-%m-%dT%H:%M:%SZ)"'","status":"STATUS","source":"SOURCE","host":"opencode","outside_provider":"codex","outside_status":"OUTSIDE_STATUS","phase":"design","commit":"'"$(git rev-parse --short HEAD)"'"}'
```
