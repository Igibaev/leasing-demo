...
Use AskUserQuestion:
> "Want outside design voices before the detailed review? Codex evaluates against OpenAI's design hard rules + litmus checks; opencode (in-host) subagent does an independent completeness review."
>
...
Output headers: `CODEX SAYS (design critique):` and `OPENCODE (IN-HOST) SUBAGENT (design completeness):`.
...
═══════════════════════════════════════════════════════════════
  Check                                    opencode (in-host)  Codex  Consensus
  ─────────────────────────────────────── ─────── ─────── ─────────
...
is no longer in context, Read `~/.config/opencode/skills/gstack/plan-design-review/SKILL.md`
...
reconciliation from the Section self-check in `~/.config/opencode/skills/gstack/plan-design-review/SKILL.md` (Read it if no longer in context). Export only agreed implementation work; retain unapproved remedies as pending findings.
...
```bash
~/.config/opencode/skills/gstack/bin/gstack-review-log '{"skill":"plan-design-review","timestamp":"TIMESTAMP","status":"STATUS","initial_score":N,"overall_score":N,"unresolved":N,"decisions_made":N,"commit":"COMMIT"}'
```
