...
```bash
$GSTACK_BIN/gstack-question-log '{"skill":"landing-report","question_id":"<id>","question_summary":"<short>","category":"<approval|clarification|routing|cherry-pick|feedback-loop>","door_type":"<one-way|two-way>","options_count":N,"user_choice":"<key>","recommended":"<key>","session_id":"SESSION_ID"}' 2>/dev/null || true
```
...
```bash
$GSTACK_BIN/gstack-skill-end --skill "landing-report" --outcome OUTCOME \
...
for LEVEL in micro patch minor major; do
  bun run ~/.config/opencode/skills/gstack/bin/gstack-next-version \
    --base "$BASE_BRANCH" \
    --bump "$LEVEL" \
