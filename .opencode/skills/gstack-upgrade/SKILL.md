...
[ -z "$_AUTO" ] && _AUTO=$(~/.config/opencode/skills/gstack/bin/gstack-config get auto_upgrade 2>/dev/null || true)
...
```bash
~/.config/opencode/skills/gstack/bin/gstack-config set auto_upgrade true
```
...
**If "Never ask again":**
```bash
~/.config/opencode/skills/gstack/bin/gstack-config set update_check false
```
Tell user: "Update checks disabled. Run `~/.config/opencode/skills/gstack/bin/gstack-config set update_check true` to re-enable."
Continue with the current skill.

### Step 2: Detect install type

```bash
if [ -d "$HOME/.opencode/skills/gstack/.git" ]; then
  INSTALL_TYPE="global-git"
  INSTALL_DIR="$HOME/.opencode/skills/gstack"
elif [ -d "$HOME/.gstack/repos/gstack/.git" ]; then
  INSTALL_TYPE="global-git"
  INSTALL_DIR="$HOME/.gstack/repos/gstack"
elif [ -d ".opencode/skills/gstack/.git" ]; then
  INSTALL_TYPE="local-git"
  INSTALL_DIR=".opencode/skills/gstack"
elif [ -d ".agents/skills/gstack/.git" ]; then
  INSTALL_TYPE="local-git"
  INSTALL_DIR=".agents/skills/gstack"
elif [ -d ".opencode/skills/gstack" ]; then
  INSTALL_TYPE="vendored"
  INSTALL_DIR=".opencode/skills/gstack"
elif [ -d "$HOME/.opencode/skills/gstack" ]; then
...
if mv "$TMP_DIR/gstack" "$INSTALL_DIR"; then
  if (cd "$INSTALL_DIR" && ./setup --host opencode); then
...
LOCAL_GSTACK=""
if [ -n "$_ROOT" ] && [ -d "$_ROOT/.opencode/skills/gstack" ]; then
  _RESOLVED_LOCAL=$(cd "$_ROOT/.opencode/skills/gstack" && pwd -P)
  _RESOLVED_PRIMARY=$(cd "$INSTALL_DIR" && pwd -P)
  if [ "$_RESOLVED_LOCAL" != "$_RESOLVED_PRIMARY" ]; then
    LOCAL_GSTACK="$_ROOT/.opencode/skills/gstack"
  fi
fi
_TEAM_MODE=$(~/.config/opencode/skills/gstack/bin/gstack-config get team_mode 2>/dev/null || echo "false")
...
```bash
cd "$_ROOT"
git rm -r --cached .opencode/skills/gstack/ 2>/dev/null || true
if ! grep -qF '.opencode/skills/gstack/' .gitignore 2>/dev/null; then
  echo '.opencode/skills/gstack/' >> .gitignore
fi
rm -rf "$LOCAL_GSTACK"
```
...
if cp -Rf "$INSTALL_DIR" "$LOCAL_GSTACK" && rm -rf "$LOCAL_GSTACK/.git" && (cd "$LOCAL_GSTACK" && ./setup --host opencode); then
...
```
Only on `LOCAL_SYNC_OK`, tell user: "Also updated vendored copy at `$LOCAL_GSTACK` — commit `.opencode/skills/gstack/` when you're ready." Otherwise stop and report the recovery outcome; do not continue migrations or announce success.
...
if [ -d "$MIGRATIONS_DIR" ]; then
  for migration in $(find "$MIGRATIONS_DIR" -maxdepth 1 -name 'v*.sh' -type f 2>/dev/null | sort -V); do
    ...
    if [ "$OLD_VERSION" != "unknown" ] && [ "$(printf '%s\n%s' "$OLD_VERSION" "$m_ver" | sort -V | head -1)" = "$OLD_VERSION" ] && [ "$OLD_VERSION" != "$m_ver" ]; then
      ...
      # ~/.gstack state) default to ~/.config/opencode/skills/gstack when unset —
...
```bash
~/.config/opencode/skills/gstack/bin/gstack-update-check --force 2>/dev/null || \
.opencode/skills/gstack/bin/gstack-update-check --force 2>/dev/null || true
```
...
**If versions differ:** follow the Step 4.5 sync bash block above to update the local copy from the primary. Tell user: "Global v{PRIMARY_VER} is up to date. Updated local vendored copy from v{LOCAL_VER} → v{PRIMARY_VER}. Commit `.opencode/skills/gstack/` when you're ready."
