...
10. **"Turn it off" / "disable"** → `~/.config/opencode/skills/gstack/bin/gstack-config set question_tuning false`
11. **"Turn it on" / "enable"** → `~/.config/opencode/skills/gstack/bin/gstack-config set question_tuning true && touch ~/.gstack/.question-tuning-prompted`
...
1. Detect contributor state (for prompt framing only, not for auto-action):
   ...
   _CONTRIB=$(~/.config/opencode/skills/gstack/bin/gstack-config get gstack_contributor 2>/dev/null || echo "false")
...
4. If A or B: enable:
   ```bash
   ~/.config/opencode/skills/gstack/bin/gstack-config set question_tuning true
   ```
...
1. Ask FIVE one-per-dimension declaration questions via individual
   ...
   **Q5 — architecture_care:** "When there's a tradeoff between 'ship now'
   ...
   ~/.config/opencode/skills/gstack/bin/gstack-developer-profile --read >/dev/null
...
```bash
~/.config/opencode/skills/gstack/bin/gstack-developer-profile --profile
```
...
4. Write:
   ```bash
   ~/.config/opencode/skills/gstack/bin/gstack-question-preference --write '{"question_id":"<id>","preference":"<never-ask|always-ask|ask-only-for-one-way>","source":"plan-tune","free_text":"<original phrase>"}'
   ```
...
```bash
~/.config/opencode/skills/gstack/bin/gstack-developer-profile --gap
```
...
```bash
~/.config/opencode/skills/gstack/bin/gstack-question-preference --stats
eval "$(~/.config/opencode/skills/gstack/bin/gstack-slug 2>/dev/null)"
eval "$(~/.config/opencode/skills/gstack/bin/gstack-paths)"
_LOG="$GSTACK_STATE_ROOT/projects/$SLUG/question-log.jsonl"
if [ -f "$_LOG" ]; then
...
~/.config/opencode/skills/gstack/bin/gstack-developer-profile --profile | bun -e "
...
"
echo '---DISTILL---'
~/.config/opencode/skills/gstack/bin/gstack-distill-free-text --status
```
...
1. Show the proposals:
   ```bash
   ~/.config/opencode/skills/gstack/bin/gstack-distill-apply --list
   ```
...
3. **On accept** (Y): apply via the bin. The skill also publishes the
   ...
   ~/.config/opencode/skills/gstack/bin/gstack-distill-apply --proposal N --gbrain-published true|false
   ```

   For `preference`:
   ```bash
   ~/.config/opencode/skills/gstack/bin/gstack-distill-apply --proposal N
   ```
...
1. Run distill:
   ```bash
   ~/.config/opencode/skills/gstack/bin/gstack-distill-free-text
   ```
...
```bash
~/.config/opencode/skills/gstack/bin/gstack-distill-free-text --background
```
