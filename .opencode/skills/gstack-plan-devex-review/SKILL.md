...
Load the "## Pass 1" section from `~/.config/opencode/skills/gstack/plan-devex-review/dx-hall-of-fame.md`
...
~/.config/opencode/skills/gstack/bin/gstack-review-read 2>/dev/null | grep plan-devex-review || echo "NO_PRIOR_DX_REVIEWS"
```
...
Load reference: Read the "## Pass 1" section from `~/.config/opencode/skills/gstack/plan-devex-review/dx-hall-of-fame.md`.
...
Load reference: Read the "## Pass 2" section from `~/.config/opencode/skills/gstack/plan-devex-review/dx-hall-of-fame.md`.
...
Load reference: Read the "## Pass 3" section from `~/.config/opencode/skills/gstack/plan-devex-review/dx-hall-of-fame.md`.
...
Load reference: Read the "## Pass 4" section from `~/.config/opencode/skills/gstack/plan-devex-review/dx-hall-of-fame.md`.
...
Load reference: Read the "## Pass 5" section from `~/.config/opencode/skills/gstack/plan-devex-review/dx-hall-of-fame.md`.
...
Load reference: Read the "## Pass 6" section from `~/.config/opencode/skills/gstack/plan-devex-review/dx-hall-of-fame.md`.
...
Load reference: Read the "## Pass 7" section from `~/.config/opencode/skills/gstack/plan-devex-review/dx-hall-of-fame.md`.
...
Load reference: Read the "## Pass 8" section from `~/.config/opencode/skills/gstack/plan-devex-review/dx-hall-of-fame.md`.
...
`~/.config/opencode/skills/gstack/plan-devex-review/dx-hall-of-fame.md`.
...
- **`disabled`** — the user turned Codex reviews off (`codex_reviews=disabled`). Skip this section entirely; do NOT fall back to a opencode (in-host) subagent — disabled means no extra review step. Print: "Codex review skipped (codex_reviews disabled). Re-enable: `gstack-config set codex_reviews enabled`."
...
if [ "$_DISABLED_REVIEW_MODE" = disabled ]; then
  "$GSTACK_BIN/gstack-review-log" '{"skill":"codex-plan-review","timestamp":"'"$(date -u +%Y-%m-%dT%H:%M:%SZ)"'","status":"skipped","source":"none","host":"opencode","outside_provider":"codex","outside_status":"disabled","phase":"plan-review","commit":"'"$(git rev-parse --short HEAD 2>/dev/null || true)"'"}'
fi
```
...
- Auth failure (stderr contains "auth", "login", "unauthorized"): "Codex auth failed. Run \`codex login\` to authenticate." Fall back to the opencode (in-host) subagent below.
- Timeout: "Codex timed out after 5 minutes." Fall back to the opencode (in-host) subagent below.
- Empty response: "Codex returned no response." Fall back to the opencode (in-host) subagent below.
...
Present findings under an `OUTSIDE VOICE (opencode (in-host) subagent):` header.
...
**Persist the result:**
```bash
~/.config/opencode/skills/gstack/bin/gstack-review-log '{"skill":"codex-plan-review","timestamp":"'"$(date -u +%Y-%m-%dT%H:%M:%SZ)"'","status":"STATUS","source":"SOURCE","host":"opencode","outside_provider":"codex","outside_status":"OUTSIDE_STATUS","phase":"plan-review","commit":"'"$(git rev-parse --short HEAD)"'"}'
```
...
```bash
~/.config/opencode/skills/gstack/bin/gstack-review-log '{"skill":"plan-devex-review","timestamp":"TIMESTAMP","status":"STATUS","initial_score":N,"overall_score":N,"product_type":"PRODUCT_TYPE","tthw_current":"TTHW_CURRENT","tthw_target":"TTHW_TARGET","mode":"MODE","persona":"PERSONA","competitive_tier":"COMPETITIVE_TIER","unresolved":N,"commit":"COMMIT"}'
```
