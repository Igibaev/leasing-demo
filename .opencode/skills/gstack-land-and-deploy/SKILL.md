...
```bash
~/.config/opencode/skills/gstack/bin/gstack-review-read 2>/dev/null
```
...
```bash
cat ~/.config/opencode/skills/gstack/review/checklist.md 2>/dev/null || echo "Checklist not found"
```
...
```bash
~/.config/opencode/skills/gstack/bin/gstack-evidence check --label tests --expect-cmd '<the project test command>' --max-age 24 --allow-paths CHANGELOG.md,VERSION,package.json,agents-digest/gstack-AGENTS.md
```
...
```bash
~/.config/opencode/skills/gstack/bin/gstack-evidence run --label tests -- 'bun test 2>&1'
```
...
```bash
~/.config/opencode/skills/gstack/bin/gstack-issue-guard pr-body
```
...
```bash
eval $(~/.config/opencode/skills/gstack/bin/gstack-diff-scope $(gh pr view --json baseRefName -q .baseRefName 2>/dev/null || echo main) 2>/dev/null)
